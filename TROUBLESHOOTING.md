...
この文書には、シメスくんの開発・運用中に発生する可能性がある問題とその解決策を記載しています。
...
6. [デプロイ関連の問題](#デプロイ関連の問題)
   - [バージョン間の環境変数の不一致](#バージョン間の環境変数の不一致)
   - [環境変数のデバッグ方法](#環境変数のデバッグ方法)
7. [Gemini PDF→Markdown 変換機能](#gemini-pdfmarkdown変換機能)
   - [Gemini 変換が開始しない](#gemini変換が開始しない)
   - [Gemini 変換がエラーになる](#gemini変換がエラーになる)
   - [変換されたテキストが不正確](#変換されたテキストが不正確)
...
**症状:**
ローカルでは動作するが、デプロイ後に機能しない場合があります。
...
デプロイ環境での環境変数のデバッグ方法は以下の通りです：

1. アプリケーションログを確認（デプロイプラットフォームのログ画面から）
...
## Gemini PDF→Markdown 変換機能

### Gemini 変換が開始しない

**問題**: Gemini 変換機能が動作しない

**解決策**:

- `.env.local`ファイルに有効な`NEXT_PUBLIC_GEMINI_API_KEY`が設定されているか確認してください。
- 選択したファイルが PDF 形式であるか確認してください。
- ブラウザコンソールでエラーメッセージを確認してください。

### Gemini 変換がエラーになる

**問題**: Gemini 変換処理中にエラーが発生する

**解決策**:

- API キーが有効であるか確認してください。
- PDF のサイズが大きすぎないか確認してください。大きなファイルの場合は、分割して処理するか、圧縮を検討してください。
- ネットワーク接続を確認してください。
- Gemini API の使用量が制限に達していないか確認してください。

### 変換されたテキストが不正確

**問題**: PDF から変換されたテキストが元のドキュメントと一致しない

**解決策**:

- PDF の品質（特に画像として保存された PDF）を確認してください。
- テキストの言語がサポートされているか確認してください。
- 特殊な書式や表が含まれている PDF は正確に変換できない場合があります。
